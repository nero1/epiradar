...
export function generateAlertPdfHtml(alert: Partial<Alert>): PdfExportResult {
  ...
  const html = `
    ...
    <head>
      <meta charset="utf-8">
      <title>EpiRadar Alert Report — ${escapeHtml(alert.pathogen ?? "Outbreak")}</title>
      <style>
    ...
    <body>
      <div class="header">
        ...
        <h1 style="margin: 8px 0; font-size: 22px;">
          ${escapeHtml(alert.pathogen ?? "Outbreak Alert")} — ${escapeHtml((alert.country_iso ?? []).join(", "))}
        </h1>
      ...
      <div class="section">
        <h2>AI Summary</h2>
        <p style="line-height:1.7;">${escapeHtml(alert.ai_summary ?? "Summary not available.")}</p>
      </div>
      ...
      <div class="section">
        ...
        <p style="font-size:13px;">
          <strong>Source:</strong> ${escapeHtml(alert.source ?? "—")}<br>
          <strong>URL:</strong> <a href="${escapeHtml(alert.source_url ?? "#")}">${escapeHtml(alert.source_url ?? "—")}</a><br>
          <strong>Published:</strong> ${alert.published_at ? new Date(alert.published_at).toLocaleDateString() : "—"}
        </p>
      </div>

      <div class="footer">
        Generated by EpiRadar on ${exportDate} · ${escapeHtml(appUrl)}<br>
