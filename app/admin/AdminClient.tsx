...
interface Stats {
  ingestion: { recentRuns: Array<{ id: string; status: string; items_ingested: number; started_at: string; errors: Record<string, unknown> }>; alertsToday: number; alertsThisWeek: number; totalActiveAlerts: number };
  users: { free: number; paid: number };
}

interface Revenue {
  mrr: { thisMonth: string; lastMonth: string; currency: string };
  subscriptions: { activePaid: number; newThisMonth: number };
...
export default function AdminClient() {
  ...
  const statBox = (label: string, value: string | number, sub?: string) => (
    <div key={label} style={{ ...card, textAlign: "center", flex: 1 }}>
  ...
  return (
    <div>
                    ...
                    <tbody>
                      {stats.ingestion.recentRuns.map((run) => (
                        <tr key={run.id} style={{ borderBottom: "1px solid var(--border)" }}>
                          ...
                          <td style={{ padding: "8px", color: "#DC2626" }}>{Object.keys(run.errors ?? {}).length}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      {tab === "users" && (
        <div>
          {actionMsg && (
            <p style={{ fontSize: 13, color: actionMsg === "Done." ? "#16A34A" : "#DC2626", marginBottom: 12 }}>
              {actionMsg}
            </p>
          )}
          <div style={card}>
                ...
                <tbody>
                  {users.map((u) => (
                    <tr key={u.id} style={{ borderBottom: "1px solid var(--border)", opacity: u.deleted_at ? 0.5 : 1 }}>
                      ...
                      <td style={{ padding: "8px", color: u.deleted_at ? "#DC2626" : "#16A34A" }}>
                        {u.deleted_at ? "Suspended" : "Active"}
                      </td>
      ...
      {tab === "themes" && (
        <div style={card}>
          ...
          <p style={{ fontSize: 13, color: "var(--text-secondary)", marginBottom: 16 }}>
            Create and activate custom UI themes. Changes apply globally.
          </p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12, marginBottom: 16 }}>
            <label style={{ fontSize: 13, color: "var(--text-primary)" }}>
              ...
              <input
                ...
                style={{ display: "block", width: "100%", marginTop: 4, padding: "8px 10px", borderRadius: 6, border: "1px solid var(--border)", background: "var(--bg-page)", color: "var(--text-primary)", fontSize: 13, boxSizing: "border-box" }}
              />
            </label>
            <label style={{ fontSize: 13, color: "var(--text-primary)" }}>
              Brand color
              <div style={{ display: "flex", gap: 8, marginTop: 4, alignItems: "center" }}>
                <input type="color" value={brandColor} onChange={(e) => setBrandColor(e.target.value)} style={{ width: 40, height: 34, borderRadius: 4, border: "1px solid var(--border)", cursor: "pointer" }} />
                <span style={{ fontSize: 12, color: "var(--text-secondary)" }}>{brandColor}</span>
              </div>
            </label>
            <label style={{ fontSize: 13, color: "var(--text-primary)" }}>
              Page background
              <div style={{ display: "flex", gap: 8, marginTop: 4, alignItems: "center" }}>
                <input type="color" value={bgPage} onChange={(e) => setBgPage(e.target.value)} style={{ width: 40, height: 34, borderRadius: 4, border: "1px solid var(--border)", cursor: "pointer" }} />
                <span style={{ fontSize: 12, color: "var(--text-secondary)" }}>{bgPage}</span>
              </div>
            </label>
            <label style={{ fontSize: 13, color: "var(--text-primary)" }}>
              Primary text
              <div style={{ display: "flex", gap: 8, marginTop: 4, alignItems: "center" }}>
                <input type="color" value={textPrimary} onChange={(e) => setTextPrimary(e.target.value)} style={{ width: 40, height: 34, borderRadius: 4, border: "1px solid var(--border)", cursor: "pointer" }} />
                <span style={{ fontSize: 12, color: "var(--text-secondary)" }}>{textPrimary}</span>
              </div>
            </label>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={handleSaveTheme} disabled={isPending} style={{ padding: "9px 18px", background: "var(--color-brand)", color: "#fff", border: "none", borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
              Save theme
            </button>
            <button onClick={() => handleActivateTheme(themeName || "custom")} disabled={isPending} style={{ padding: "9px 18px", background: "transparent", color: "var(--color-brand)", border: "1px solid var(--color-brand)", borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
              Activate
            </button>
          </div>
              ...
              <tbody>
                {audit.map((entry) => (
                  <tr key={entry.id} style={{ borderBottom: "1px solid var(--border)" }}>
                    ...
                    <td style={{ padding: "8px", fontFamily: "monospace", fontSize: 11 }}>{entry.target_user_id?.slice(0, 8) ?? "—"}…</td>
                  </tr>
                ))}
              </tbody>
            </table>
